    ...
    <div>
      ...
      <form onSubmit={(e)=>{onSubmit(e)}} className='d-flex flex-column justify-content-center'>
        <div className="mb-3">
          ...
          <input onChange ={(e)=>onPostChange(e)} type="testPst" className="form-control" id="testPost" value={newPost} data-testid="NewMessageInput"></input>
        </div>
        <button type="submit" className="btn btn-primary" data-testid="Submit">Submit</button>
      </form>
      <button onClick={() => getAllPosts()} data-testid="Get-posts">Get posts</button>
      <hr />
      <div>
